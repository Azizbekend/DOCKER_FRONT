...
export const HardwareControlle = observer(() => {
    ...
    const { commands, changeCommands, isCommand, isLoaderCommand, switchIsCommand } = hardwareModel
    ...
    const cancle = () => { setShowAvtive(false); switchIsCommand() }
        ...
        <>
            ...
            <div className="w-full mt-10 p-[0_0_50px_0]">
                ...
                {isLoaderCommand ? <Loader /> :
                    <div className={` duration-200 ${isCommand ? "opacity-50" : "opacity-100"}`}>
                        <div className="flex justify-between mb-5 border-b pb-5 gap-3">
                            <Button onClick={() => { !isCommand && setBtnCount(0) }} class={`border-2 w-full justify-center ${btnCount == 0 ? "border-[var(--clr-accent)] text-[var(--clr-accent)]" : "border-[var(--clr-border-gray)] text-[var(--clr-gray-dark)]"}`}>Пуск</Button>
                            <Button onClick={() => { !isCommand && setBtnCount(1) }} class={`border-2 w-full justify-center ${btnCount == 1 ? "border-[var(--clr-accent)] text-[var(--clr-accent)]" : "border-[var(--clr-border-gray)] text-[var(--clr-gray-dark)]"}`}>Стоп</Button>
                            <Button onClick={() => { !isCommand && setBtnCount(2) }} class={`border-2 w-full justify-center ${btnCount == 2 ? "border-[var(--clr-accent)] text-[var(--clr-accent)]" : "border-[var(--clr-border-gray)] text-[var(--clr-gray-dark)]"}`}>Cброс аварии</Button>
                        </div>
