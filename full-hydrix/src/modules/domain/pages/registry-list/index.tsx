...
const columns: TableColumn<DespetcherTest>[] = [
    ...
    },
    {
        ...
        cell: ({ volumeAverage }) => {
            return (
                <div className='text-center w-full text-gray-800 font-medium text-lg'>{volumeAverage + " м³" || '—'}</div>
            );
        },
    },
    {
        ...
        cell: ({ volumeReale }) => {
            return (
                <div className='text-center w-full text-gray-800 font-medium text-lg'>{volumeReale + " м³" || '—'}</div>
            );
        },
    },
    {
...
export const RegistryObjects = observer(({ list }: { list: DespetcherTest[] }) => {
    const handleRowClick = (row: DespetcherTest) => {
        window.location.href = `/domain/passport`;
    };
