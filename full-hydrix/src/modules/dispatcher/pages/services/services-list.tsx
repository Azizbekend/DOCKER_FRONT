...
export const RequestRegistryList = observer(() => {
  ...
  const inWork = filteredRequests.filter(r => r.status === 'В работе').length;
  const awaiting = filteredRequests.filter(r => r.status === 'Ожидание').length;
  const completed = filteredRequests.filter(r => r.status === 'Завершено').length;
  ...
  return isLoader ? <Loader /> : (
    <>
      <ServiceStagesPanel 
        show={isStagesPanel} 
        onClose={() => setIsStagesPanel(false, 0)} 
      ...
      {/* Панель отладки (скрыта в продакшене) */}
      {isLook && (
...
<div className="space-y-5">
  {filteredRequests.length > 0 ? (
    filteredRequests.map((item) => (
      <div 
        key={item.id}
        onClick={() => setIsStagesPanel(true, item.id)}
      ...
      >
        ...
        <div className="flex items-start justify-between gap-4 mb-4">
          <div className="min-w-0">
            ...
            <div className="text-sm text-gray-600 font-mono">ID: {item.id}</div>
          </div>
