...
export const EquipmentPassport = observer(() => {
    const { getInfoNodeInfoAll, model, documents, сharacteristic, commandsInfo, clickTest } = hardwareModel;
                            ...
                            {commandsInfo.map((item, key) => {
                                return (
                                    <div className={`info-comp__item ${commandsInfo.length > 1 && "border-b border-gray-300 pb-4"}`} key={key}>
                                        ...
                                        <div className='flex'>
                                            <div className="info-comp__description">{formatToTwoDecimalsSafe(item.value)}</div>
